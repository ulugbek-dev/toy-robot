...
import { TableAndRobotStyled } from './TableAndRobotStyled';
import { useSelector } from 'react-redux';

export default function TableAndRobot() {

    // Get robot's current position from store
    ...
    // Table cells length
    const tableArray = [...Array(5).keys()];

    return (
        <TableAndRobotStyled 
            position={position.split(',')}
            direction={direction(position.split(',')[2])}
            ...
            <div className="table">
                {tableArray.map(r => (
                    <div className="row" key={r}>
                        {tableArray.map(c => (
                            <div className="cell" key={c}></div>
                        ))}
                    </div>
                ))}
                <svg className="robot" viewBox="0 0 512 512">
                <path
                    fill="#A9A9A9"
